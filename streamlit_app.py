...
def load_airlines():
    ...
    # expected columns: "State" and "Aeroplane Operator Name"
...
# LOAD
# =====================
baseline = load_baseline()  # not used in charts yet, but kept for future
...
st.caption("Klik peta atau gunakan dropdown. Ada mode All vs CORSIA-subject, plus ranking pairs & countries. Airlines = info only.")

# =====================
# SIDEBAR CONTROLS (ALL KEYS UNIQUE)
# =====================
with st.sidebar:
    st.header("Controls")
...
with map_col:
    ...
    # Streamlit returns selection dict in newer versions when on_select is used.
...
"""
            ...
            with c1:
                ...
                if a_list:
                    ...
                    st.warning("No airline list found for this country name. If unexpected, add alias in COUNTRY_ALIAS.")
            with c2:
    ...
    with tabs[2]:
        if country_focus == "—":
            ...
            if filtered:
                ...
                st.info("No airlines (or no match for your search). If expected, add alias in COUNTRY_ALIAS.")
...
- **All emissions**: subject + not subject (total international aviation CO₂ in dataset).
- **CORSIA-subject only**: hanya baris yang `subject=True`.
- **Country involvement**: outgoing + incoming (asal + tujuan).
- **Airlines**: directory tambahan dari file attribution (info only).
- Klik bar chart pada Rankings/Country view untuk otomatis memilih route (A→B).
- Jika beberapa negara tidak muncul di daftar maskapai, tambahkan mapping di `COUNTRY_ALIAS`.
        """
    )
