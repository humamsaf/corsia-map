...
def split_pair(val: str, delim: str):
    ...
    s = str(val).strip()
    parts = s.split(delim)
    if len(parts) != 2: return (s, None)
    return parts[0].strip(), parts[1].strip()

@st.cache_data
def load_baseline_2019():
    raw = pd.read_excel(BASELINE_XLSX, sheet_name=0, header=None)
    start_idx = raw.index[raw[0].astype(str).str.contains("Afghanistan", na=False)][0]
...
@st.cache_data
def load_current_2024():
    raw = pd.read_excel(CURRENT_XLSX, sheet_name=0, header=None)
    start_idx = raw.index[raw[0].astype(str).str.contains("Afghanistan", na=False)][0]
    data = raw.iloc[start_idx:].copy()
...
def load_attribution():
    try:
        df = pd.read_excel(ATTR_XLSX, sheet_name="Attributions")
    except Exception:
        return pd.DataFrame()
    rename_map = {
        "State": "state",
...
baseline = load_baseline_2019()
current  = load_current_2024()
...
with map_col:
    ...
    def role(c):
        if st.session_state.A == c: return "A"
        if st.session_state.B == c: return "B"
        return "Other"
    ...
    clicked = plotly_events(fig_map, click_event=True, hover_event=False, select_event=False, key="map_click_evt")
...
with panel_col:
    ...
    fig1 = px.bar(
        pd.DataFrame({"Scenario":[f"Baseline {baseline_mode}","Current 2024"], "Emissions (tCO₂)":[baseline_used,current_total]}),
        x="Scenario", y="Emissions (tCO₂)", text_auto=True, title="Current vs Baseline"
    )
    st.plotly_chart(fig1, use_container_width=True)

    fig2 = px.bar(
        pd.DataFrame({"Row":["Total","Total"], "Category":["Subject","Not subject"], "Emissions (tCO₂)":[subject,not_subject]}),
        x="Emissions (tCO₂)", y="Row", color="Category", orientation="h", text_auto=True,
    ...
    with tabs[1]:
        st.markdown(
            """
- Map is a selector (navigator). Numbers live in the panel (explainer).
- Baseline from “Pilot phase – based on 2019 data”.
- Current 2024 split into subject vs not subject.
            """
        )
